...
from ccxt.tokenomika_assets import (
    TESTNET_ASSETS,
    TESTNET_PAIRS,
    MAINNET_ASSETS,
    MAINNET_PAIRS,
...
class wavesexchange(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(wavesexchange, self).describe(), {
            ...
            'urls': {
                ...
                "test": {
                    'matcher': "https://dev-waves-dex.tkn-dev.com",
                    'node': "https://dev-waves-node.tkn-dev.com",
                ...
                'api': {
                    'matcher': 'https://waves-dex.tokenomica.com',
    ...
    def fetch_markets(self, params={}):
        ...
        for p in pairs:
            # entry = response[i]
            baseId = self.safe_string(assets[p[0]], 'id')
            quoteId = self.safe_string(assets[p[1]], 'id')
            id = baseId + '/' + quoteId
            # marketId = self.safe_string(entry, 'symbol')
            base, quote = p
            symbol = self.safe_currency_code(base) + '/' + self.safe_currency_code(quote)
            precision = {
                'amount': self.safe_integer(assets[p[0]], 'precision'),
                'price': self.safe_integer(assets[p[1]], 'precision'),
            }
            result.append({
                'symbol': symbol,
                'id': id,
    ...
    def fetch_deposit_address(self, code, params={}):
        ...
        if not (code in currencies):
            ...
            raise ExchangeError(
                self.id + ' fetch ' + code + ' deposit address not supported. Currency code must be one of ' + str(
                    codes))
        request = self.extend({
            'code': code,
        }, params)
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        signature = self.eddsa(base64.b16encode(binary), base64.b16encode(self.base58_to_binary(self.secret)),
                               'ed25519')
        assetPair = {
    ...
    def cancel_order(self, id, symbol=None, params={}):
        ...
        response = self.matcherPostMatcherOrderbookBaseIdQuoteIdCancel(self.extend(request, params))
        # {orderId: 'Do7cDJMf2MJuFyorvxNNuzS42MXSGGEq1r1hGDn1PHiS',
        #   success: True,
        #   status: 'OrderCanceled'}
        returnedId = self.safe_string(response, 'orderId')
        return {
            'info': response,
            'id': returnedId,
            'clientOrderId': None,
    ...
    def withdraw(self, code, amount, address, tag=None, params={}):
        ...
        if code != 'WAVES':
            ...
            if not (code in currencies):
                codes = list(currencies.keys())
                raise ExchangeError(
                    self.id + ' fetch ' + code + ' withdrawals are not supported. Currency code must be one of ' + str(
                        codes))
        self.load_markets()
